...
- On macOS showcase setups, the lane now treats a successful upstream `SNIO` server probe as sufficient for `usable=true`, even when the older `runtime_probe.sio` CLI smoke is unavailable; this keeps the report aligned with the official `Sounio` interop contract instead of the legacy probe path.
- The preferred macOS showcase binary path is now the Lima wrapper at [/Users/demetriosagourakis/Documents/New project/scripts/sounio-lima-souc](/Users/demetriosagourakis/Documents/New%20project/scripts/sounio-lima-souc), which can run an upstream Linux `souc` release under Lima when a native Darwin `souc` lags the source tree.
